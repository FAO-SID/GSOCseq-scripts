...
#Open empty vector
WARM_UP<-vect("INPUTS/TARGET_POINTS/THA_NP_Target_points.shp")


#Open Warm Up Stack
Stack_Set_warmup<- rast("INPUTS/STACK/THA_NP_Stack_Set_Warm_UP.tif")

# Open Result from the spin up phase
#A vector with 5 columns, one for each pool
Spin_up<-vect("OUTPUTS/1_SPIN_UP/THA_NP_SPIN_UP.shp")
...
PREC<-rast("INPUTS/TERRA_CLIMATE/Precipitation_2001-2020.tif")
TEMP<-rast("INPUTS/TERRA_CLIMATE/AverageTemperature_2001-2020.tif")*0.1
PET<-rast("INPUTS/TERRA_CLIMATE/PET_2001-2020.tif")*0.1
...
NPP<-rast("INPUTS/NPP_TERRA/NPP_MIAMI_MEAN_81-00_AOI.tif")

NPP_MEAN_MIN<-rast("INPUTS/NPP_TERRA/NPP_MIAMI_MEAN_81-00_AOI_MIN.tif")
...
# Number of layers of stack without climatic and NPP layers
nl_stack <- dim(Stack_Set_warmup)[3]


# Generate the input vector for the RothC model
clim <- c(TEMP,PREC,PET)
clim <- resample(clim, LU_AOI,method='bilinear')
...
# Generate the input vector for the RothC model
# clim <- c(TEMP,PREC,PET)
# NPP_all <- c(NPP,NPP_MEAN_MIN,NPP_MEAN_MAX)
# # Extract variables to points
# Vector_points<-cbind(WARM_UP$FID, extract(Stack_Set_warmup,WARM_UP, ID=F),
#                      extract(clim,WARM_UP, ID=F),
#                      extract(NPP_all,WARM_UP,ID=F,xy=T))
# Vector_points <-vect(Vector_points, geom=c('x','y'))
# rm(NPP_all)
...
nppBand<-nl_stack+clim_layers*3+2

firstClimLayer<-nl_stack+2
...
rm(list=c('clim','LU_AOI','LU_stack','NPP','NPP_MEAN_MIN','NPP_MEAN_MAX', 'TEMP','PREC','PET','Stack_Set_warmup'))
gc()
...
writeOGR(WARM_UP,"OUTPUTS/2_WARM_UP/", paste0(ISO, "_WARM_UP"), driver="ESRI Shapefile",overwrite=TRUE) 
