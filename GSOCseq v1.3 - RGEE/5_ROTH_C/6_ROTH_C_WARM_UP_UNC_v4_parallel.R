...
#Open empty vector
#Vector<-readOGR(paste0("INPUTS/TARGET_POINTS/target_points_",AOI,".shp"))


# Points for the spin-up phase ----
...
temp <-read_sf(paste0('INPUTS/TERRA_CLIMATE/avT_warmup_up_',AOI,'.shp'))
prec <-read_sf(paste0('INPUTS/TERRA_CLIMATE/pr_warmup_up_',AOI,'.shp'))
pet <-read_sf(paste0('INPUTS/TERRA_CLIMATE/pet_warmup_up_',AOI,'.shp'))

NPP <- vect(paste0('INPUTS/NPP_TERRA/MEAN_NPP_81_01_',AOI,'.shp'))

NPP_MEAN_MIN <-vect(paste0('INPUTS/NPP_TERRA/MINIMUM_NPP_81_01_',AOI,'.shp'))

NPP_MEAN_MAX<-vect(paste0('INPUTS/NPP_TERRA/MAXIMUM_NPP_81_01_',AOI,'.shp'))
...
lu <- lu[, c("id" ,"remapped","DR" )]
#Apply NPP coefficientes
NPP$mean_npp<-(lu$remapped==2 | lu$remapped==12 | lu$remapped==13)*NPP$mean_npp*0.53+ (lu$remapped==4)*NPP$mean_npp*0.88 + (lu$remapped==3 | lu$remapped==5 | lu$remapped==6 | lu$remapped==8)*NPP$mean_npp*0.72
NPP_MEAN_MIN$mean_npp<-(lu$remapped==2 | lu$remapped==12 | lu$remapped==13)*NPP_MEAN_MIN$mean_npp*0.53+ (lu$remapped==4)*NPP_MEAN_MIN$mean_npp*0.88 + (lu$remapped==3 | lu$remapped==5 | lu$remapped==6 | lu$remapped==8)*NPP_MEAN_MIN$mean_npp*0.72
NPP_MEAN_MAX$mean_npp<-(lu$remapped==2 | lu$remapped==12 | lu$remapped==13)*NPP_MEAN_MAX$mean_npp*0.53+ (lu$remapped==4)*NPP_MEAN_MAX$mean_npp*0.88 + (lu$remapped==3 | lu$remapped==5 | lu$remapped==6 | lu$remapped==8)*NPP_MEAN_MAX$mean_npp*0.72
...
for (i in 1:Years){
  ...
  lu[, paste0('remapped',i)] <- lu$remapped
  
}
for (i in 1:Years){
  
  lu[, paste0('DR',i)] <- lu$DR
}
lu[,'remapped'] <-NULL
lu[,'DR'] <-NULL
...
cov <-st_as_sf(cov)
NPP <-st_as_sf(NPP)
NPP_MEAN_MIN<-st_as_sf(NPP_MEAN_MIN)
NPP_MEAN_MAX<-st_as_sf(NPP_MEAN_MAX)

#Combine all data points

dat <- st_join(soil,cov)
dat <- st_join(dat,lu)
dat <- st_join(dat,temp)
dat <- st_join(dat,prec)
dat <- st_join(dat,pet)
dat <- st_join(dat,NPP)
dat <- st_join(dat,NPP_MEAN_MIN)
dat <- st_join(dat,NPP_MEAN_MAX)

rm(soil, lu,cov,prec,pet,temp,NPP,NPP_MEAN_MAX,NPP_MEAN_MIN)
gc()

cols <- grep('id', names(dat), value=T)
dat[, c("id.y", "id.x.1", "id.y.1" ,"id.x.2", "id.y.2")] <-NULL
names(dat)[1] <-"ID" 
dat[is.na(dat)] <-0
dat$ID<-as.integer(dat$ID)
#write_sf(dat, paste('INPUTS/STACK/Warm_up_',AOI,'.shp'))


# Open Result from the spin up phase
#A vector with 5 columns, one for each pool
Spin_up<-read_sf(paste0("OUTPUTS/1_SPIN_UP/SPIN_UP_",AOI,".shp"))

#Spin_up<-Spin_up[complete.cases(Spin_up$Cnpt_EQ),]
Spin_up <-  as(Spin_up, "Spatial")
IDs <- Spin_up$ID

Vector_points <-dat[dat$ID %in% IDs,]
Vector_points <-  as(Vector_points, "Spatial")
    ...
    iteration<-function(Vector_variables2,WARM_UP2,Spin_up2)
    {
      ...
      for (i in 1:nrow(Vector_variables2@data)) 
      {
        ...
        for (w in 1:Years) {
          ...
          LU_im<-as.data.frame(t(Vect[nLU_beg:nLU_end])) # LU one per year according to LU
          ...
          if (any(is.na(Evp[,2])) | any(is.na(Temp[,2])) | any(is.na(SOC_im)) | any(is.na(clay_im)) | any(is.na(Spin_up@data[i,3]))  | any(is.na(NPP_im)) | any(is.na(Precip[,2]))  |  any(is.na(Cov2[,2]))  |  any(is.na(Cov1[,1]))  | any(is.na(DR_im[,1]))    |  (SOC_im<0) | (clay_im<0) | (Spin_up@data[i,3]<=0) ) {WARM_UP2[i,2]<-0}else{
            ...
            if (w==1) {
              f_wp<-Roth_C(Cinputs=Cinputs[1],years=years,DPMptf=Spin_up2@data[i,5], RPMptf=Spin_up2@data[i,6], BIOptf=Spin_up2@data[i,7], HUMptf=Spin_up2@data[i,8], FallIOM=Spin_up2@data[i,9],Temp=Temp,Precip=Precip,Evp=Evp,Cov=Cov,Cov1=Cov1,Cov2=Cov2,soil.thick=soil.thick,SOC=SOC,clay=clay,DR=DR,bare1=bare1,LU=LU)
            } else {
...
setwd("OUTPUTS/2_WARM_UP")
writeOGR(WARM_UP,".", paste0("WARM_UP_",AOI), driver="ESRI Shapefile",overwrite=TRUE)
