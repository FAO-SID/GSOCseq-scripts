...
WD_FOLDER  <- 'C:/Users/hp/Documents/FAO/GSOCseq/Lebanon'
...
clim <-vect(paste0('INPUTS/TERRA_CLIMATE/Spin_up_',AOI,'.shp'))
#Reoder columns
## Temp
...
#write_sf(dat, paste('INPUTS/STACK/Spin_up_',AOI,'.shp'))

# Vector must be an empty points vector. 
C_INPUT_EQ<-dat[, 'ID']

C_INPUT_EQ <- as(C_INPUT_EQ, "Spatial")
Vector_variables<-as.data.frame(vect(dat))
rm(dat)
gc()
...
Vector_variables <-round_df(Vector_variables, digits=2)
...
writeOGR(C_INPUT_EQ, ".", paste0("SPIN_UP_",AOI), driver="ESRI Shapefile",overwrite=TRUE) 
