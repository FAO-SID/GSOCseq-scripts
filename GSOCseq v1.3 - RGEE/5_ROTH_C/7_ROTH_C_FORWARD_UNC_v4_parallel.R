...
working_dir<-setwd("C:/Users/hp/Documents/FAO/GSOCseq/Lebanon")
WD_OUT<-("/OUTPUTS/3_FOWARD")



# OPEN THE RESULT VECTOR FROM THE WARM UP PROCESS

WARM_UP<-read_sf(paste0("OUTPUTS/2_WARM_UP/WARM_UP_",AOI,".shp"))
Vector <- WARM_UP[, c('ID', 'geometry')]
 
WARM_UP <-as(WARM_UP, 'Spatial')
Vector <-as(Vector, 'Spatial') 
IDs <- WARM_UP@data$ID
...
# Points for the forward phase ----
# Load Soil properties
soil <- vect(paste0('INPUTS/SOIL/SOC_clay_',AOI,'.shp'))

#Load Tem, Prec and Pet
clim <-vect(paste0('INPUTS/TERRA_CLIMATE/Forward_up_',AOI,'.shp'))
#Reoder columns
## Temp
(grep('tmm', names(clim), value = T))
temp <- c("0_tmmx","1_tmmx" , "2_tmmx",  "3_tmmx",  "4_tmmx",  "5_tmmx",  "6_tmmx" , "7_tmmx" ,
          "8_tmmx" , "9_tmmx", "10_tmmx", "11_tmmx" )
(grep('pr', names(clim), value = T))
pr <- c("0_pr","1_pr" , "2_pr",  "3_pr",  "4_pr",  "5_pr",  "6_pr" , "7_pr" ,
        "8_pr" , "9_pr", "10_pr", "11_pr" )

(grep('pet', names(clim), value = T))
pet <- c("0_pet","1_pet" , "2_pet",  "3_pet",  "4_pet",  "5_pet",  "6_pet" , "7_pet" ,
         "8_pet" , "9_pet", "10_pet", "11_pet" )

cols <- c('id',temp,pr,pet)
clim <- clim[,cols]

# Load LU and add DR columns
#DR<-(LU_AOI==2 | LU_AOI==12| LU_AOI==13)*1.44+ (LU_AOI==4)*0.25 + (LU_AOI==3 | LU_AOI==5 | LU_AOI==6 | LU_AOI==8)*0.67
lu <- vect(paste0('INPUTS/TARGET_POINTS/target_points_',AOI,'.shp'))

lu$DR <- (lu$remapped==2 | lu$remapped==12| lu$remapped==13)*1.44+ (lu$remapped==4)*0.25 + (lu$remapped==3 | lu$remapped==5 | lu$remapped==6 | lu$remapped==8)*0.67
lu <- lu[, c("id" ,"DR","remapped" )]

# Vegetations cover
cov <- vect(paste0('INPUTS/COV/veg_COV_',AOI,'.shp'))
cols <- c( "id", "X0_count" , "X1_count" , "X2_count"  ,"X3_count" , "X4_count" ,
           "X5_count" , "X6_count" , "X7_count" , "X8_count" , "X9_count","X10_count", "X11_count" )
cov <- cov[, cols]             

#stack(SOC_MAP_AOI,Clay_WA_AOI_res,TEMP_AOI,PREC_AOI,PET_AOI,DR,LU_AOI,Cov_AOI)
#Combine all points in precise order
#convert vect to sf for spatial union
soil <- st_as_sf(soil)
clim <-st_as_sf(clim)
lu <-st_as_sf(lu)
cov <-st_as_sf(cov)

dat <- st_join(soil,clim)
dat <- st_join(dat,lu)
dat <- st_join(dat,cov)
...
cols <- grep('id', names(dat), value=T)
dat[, c("id.y" ,  "id.x.1" ,"id.y.1")] <-NULL
names(dat)[1] <-"ID" 
dat[is.na(dat)] <-0

dat <- dat[dat$ID %in% IDs,]

Variables <-as(dat, 'Spatial')
rm(dat)
...
results <- foreach(j=1:length(listOfRows),.inorder = FALSE,.combine = rbind,.packages= c('raster','rgdal',"SoilR","soilassessment")) %dopar% {
  ...
  iteration<-function(Vector_variables2,FOWARD2,WARM_UP2)
  {
...
if (any(is.na(Evp[,2])) | any(is.na(Temp[,2])) | any(is.na(SOC_im)) | any(is.na(clay_im)) | any(is.na(Precip[,2]))  |  any(is.na(Cov2[,2]))  |  any(is.na(Cov1[,1])) | any(is.na(Cinputs_im)) | any(is.na(DR_im)) | (Cinputs_im<0) |  (SOC_im<0) | (clay_im<0) ) {FOWARD2[i,2]<-0}else{
...
FOWARD2[i,8]<-f_bau[5]
FOWARD2[i,9]<-LU
FOWARD2[i,10]<-f_low_t
FOWARD2[i,11]<-f_med_t
FOWARD2[i,12]<-f_high_t
FOWARD2[i,13]<-f_bau_t_min
FOWARD2[i,14]<-f_bau_t_max
FOWARD2[i,15]<-f_med_t_min
FOWARD2[i,16]<-f_med_t_max
FOWARD2[i,17]<-SOC_t0_min
FOWARD2[i,18]<-SOC_t0_max
...
#print(c(i,SOC,f_bau_t,f_low_t,f_med_t,f_high_t,f_bau_t_min,f_bau_t_max))
  ...
  x<- iteration(Vector_variables2=listOfRows[[j]],FOWARD2=listOfCin[[j]],WARM_UP2=listOfWup[[j]])
  x
}
...
setwd('OUTPUTS/3_FOWARD/') 
writeOGR(FOWARD, ".", paste0("FORWARD_",AOI), driver="ESRI Shapefile",overwrite=TRUE) 
